...
@click.option("-p", "--plot", is_flag=True, help="Generate visualization plots.")
@click.option(
    "--plot-dir",
    type=click.Path(),
...
@click.option("--classify", is_flag=True, help="Add excimer/monomer classification.")
@click.option(
    "--no-shapely",
    is_flag=True,
...
def analyze(
    ...
    if plot:
        ...
        try:
            ...
            plot_conformer_distribution(results, output=plot_path / "distributions.png")
            if not quiet:
                click.echo(f"Saved: {plot_path / 'distributions.png'}")

            create_summary_figure(results, output=plot_path / "summary.png")
            if not quiet:
    ...
    if not quiet:
        ...
        click.echo(
            f"Plane angle range: {results['plane_angle_deg'].min():.1f} - "
            f"{results['plane_angle_deg'].max():.1f}°"
        )
        click.echo(
            f"Distance range: {results['interplane_distance_A'].min():.2f} - "
...
@click.option(
    "-n", "--num", default=5, type=int, help="Number of conformers to display."
)
        ...
        if n_confs > 0:
            ...
            try:
                ...
                for i in range(min(num, n_confs)):
                    ...
                    click.echo(
                        f"  {i:>4} {result['plane_angle_deg']:>8.1f} "
