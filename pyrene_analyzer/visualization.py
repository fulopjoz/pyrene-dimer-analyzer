...
def plot_angle_vs_energy(
    ...
    if show_excimer_threshold:
        ax.axvline(x=20, color='green', linestyle='--', alpha=0.5, linewidth=2,
                   label='Strong excimer threshold')
        ax.axvline(x=60, color='orange', linestyle='--', alpha=0.5, linewidth=2,
    ...
    if title:
        ...
        ax.set_title('Plane Angle vs Energy for Pyrene Dimer Conformers',
...
def plot_distance_vs_overlap(
    ...
    if title:
        ...
        ax.set_title('Inter-plane Distance vs π-Overlap',
                     fontsize=14, fontweight='bold')
...
def plot_conformer_distribution(
    ...
    for idx, param in enumerate(parameters):
        ...
        ax.set_title(f'Distribution of {labels.get(param, param)}',
...
def plot_variant_comparison(
    ...
    ax.set_title(f'Comparison of {labels.get(parameter, parameter)} Across Variants',
                 fontsize=14, fontweight='bold')
...
def plot_correlation_matrix(
    ...
    ax.set_title('Correlation Matrix of Geometric Parameters',
                 fontsize=14, fontweight='bold')
...
def create_summary_figure(
    ...
    ax1.hist(df['plane_angle_deg'].dropna(), bins=20, edgecolor='white',
             alpha=0.8, color=sns.color_palette("husl")[0])
    ax1.set_xlabel('Plane Angle θ (degrees)')
    ax1.set_ylabel('Count')
    ax1.set_title('Angle Distribution')
    ax1.axvline(df['plane_angle_deg'].mean(), color='red', linestyle='--',
                label=f'Mean: {df["plane_angle_deg"].mean():.1f}°')
    ax1.legend()
    
    # Panel 2: Distance distribution
    ax2 = fig.add_subplot(gs[0, 1])
    ax2.hist(df['interplane_distance_A'].dropna(), bins=20, edgecolor='white',
             alpha=0.8, color=sns.color_palette("husl")[1])
    ax2.set_xlabel('Inter-plane Distance d (Å)')
    ax2.set_ylabel('Count')
    ax2.set_title('Distance Distribution')
    ax2.axvline(df['interplane_distance_A'].mean(), color='red', linestyle='--',
                label=f'Mean: {df["interplane_distance_A"].mean():.2f} Å')
    ax2.legend()
    
    # Panel 3: Overlap distribution
    ax3 = fig.add_subplot(gs[0, 2])
    ax3.hist(df['pi_overlap_pct'].dropna(), bins=20, edgecolor='white',
             alpha=0.8, color=sns.color_palette("husl")[2])
    ax3.set_xlabel('π-Overlap (%)')
    ax3.set_ylabel('Count')
    ax3.set_title('Overlap Distribution')
    ax3.axvline(df['pi_overlap_pct'].mean(), color='red', linestyle='--',
                label=f'Mean: {df["pi_overlap_pct"].mean():.1f}%')
    ax3.legend()
    
    # Panel 4: Angle vs Distance scatter
    ax4 = fig.add_subplot(gs[1, 0])
    scatter = ax4.scatter(df['plane_angle_deg'], df['interplane_distance_A'],
                          c=df['pi_overlap_pct'], cmap='viridis', alpha=0.7, s=40)
    ...
    scatter = ax5.scatter(df['interplane_distance_A'], df['pi_overlap_pct'],
                          c=df['plane_angle_deg'], cmap='coolwarm', alpha=0.7, s=40)
    ...
    if 'classification' in df.columns:
        ...
        stats = df[['plane_angle_deg', 'interplane_distance_A', 'pi_overlap_pct']].describe()
        ax6.axis('off')
        table = ax6.table(
    ...
    plt.suptitle('Pyrene Dimer Conformational Analysis Summary',
